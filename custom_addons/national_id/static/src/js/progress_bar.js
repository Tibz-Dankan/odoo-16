...
FieldProgressBar.include({
  ...
  _render_value: function(v) {
    ...
    var progressBarComplete = this.$(".o_progressbar_complete");
    progressBarComplete.toggleClass("o_progress_lt_fty", widthComplete <= 50);
    progressBarComplete.toggleClass(
      "o_progress_gt_fty",
      widthComplete > 50 && widthComplete < 100
    );
    progressBarComplete.css("width", widthComplete + "%");

    if (this.readonly) {
      if (max_value !== 100) {
        this.$(".o_progressbar_value").html(
          utils.human_number(value) +
            " / " +
            utils.human_number(max_value)
        );
      } else {
